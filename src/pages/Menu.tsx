...
const Menu = () => {
  ...
  useEffect(() => {
    supabase.from("menu_items").select("*").order("created_at").then(({ data }) => setMenuItems(data || []));
    if (user) {
      supabase.from("bookings").select("*").eq("user_id", user.id).eq("status", "active")
        .order("created_at", { ascending: false }).limit(1)
        .then(({ data }) => { if (data && data.length > 0) setActiveBooking(data[0]); });
    }
  }, [user]);

  const categories = ["الكل", ...Array.from(new Set(menuItems.map((i: any) => i.category)))];
  const filtered = activeCategory === "الكل" ? menuItems : menuItems.filter((i: any) => i.category === activeCategory);
  ...
  const handleSend = async () => {
    if (!activeBooking) {
      toast({ title: "تم إرسال الطلب للمطبخ! 🍳", description: `${cartCount} أصناف - ${cartTotal} ج.م` });
      setCart({});
      setShowCart(false);
      return;
    }
    ...
    const orderItems = Object.entries(cart).map(([id, qty]) => {
      const item = menuItems.find((i: any) => i.id === id)!;
      return { booking_id: activeBooking.id, item_name: item.name, item_price: item.price, quantity: qty };
    });
    await supabase.from("booking_orders").insert(orderItems);

    // Update booking total
    const newOrdersCost = (activeBooking.orders_cost || 0) + cartTotal;
    const newTotal = (activeBooking.area_cost || 0) + newOrdersCost;
    await supabase.from("bookings").update({ orders_cost: newOrdersCost, total_cost: newTotal }).eq("id", activeBooking.id);
    setActiveBooking({ ...activeBooking, orders_cost: newOrdersCost, total_cost: newTotal });

    setOrderSent(true);
    toast({ title: "تم إرسال الطلب للمطبخ! 🍳", description: `إجمالي الفاتورة: ${newTotal} ج.م` });
    setCart({});
    setShowCart(false);
  };
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex-1">
              ...
              <p className="text-xs font-cairo text-muted-foreground">
                {activeBooking.type === "public" ? `مساحة عامة: ${activeBooking.area_cost} ج.م` : activeBooking.room_name}
                {activeBooking.orders_cost > 0 && ` | طلبات: ${activeBooking.orders_cost} ج.م`}
              </p>
            </div>
            <div className="text-left">
              <p className="text-lg font-bold text-gradient-gold font-cairo">{activeBooking.total_cost} ج.م</p>
            </div>
          </motion.div>
        </div>
      )}
      ...
      {orderSent && activeBooking && (
        <div className="px-6 mt-3">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <p className="font-cairo font-bold text-emerald text-sm">✅ تم إرسال طلبك! الفاتورة الإجمالية: {activeBooking.total_cost} ج.م</p>
            <div className="flex gap-2 mt-3 justify-center">
              <Button variant="outline" size="sm" className="font-cairo text-xs" onClick={() => setOrderSent(false)}>طلب المزيد</Button>
              <Button variant="hero" size="sm" className="font-cairo text-xs" onClick={() => navigate("/home")}>العودة للرئيسية</Button>
            </div>
          </motion.div>
        </div>
      )}
          ...
          {categories.map((cat) => (
            <button key={cat} onClick={() => setActiveCategory(cat)}
              className={`px-4 py-2 rounded-full whitespace-nowrap font-cairo text-sm font-medium transition-all ${
                activeCategory === cat ? "gradient-gold text-emerald-dark shadow-gold" : "bg-card text-muted-foreground border border-border"
              }`}>{cat}</button>
          ))}
        </div>
      </div>
          ...
          {showCart ? (
            <div className="bg-card rounded-2xl p-6 border border-border shadow-2xl">
              ...
              <div className="border-t border-border pt-3 flex justify-between mb-4">
                <span className="font-bold text-card-foreground font-cairo">الإجمالي</span>
                <span className="font-bold text-gradient-gold font-cairo text-lg">
                  {activeBooking ? (activeBooking.total_cost + cartTotal) : cartTotal} ج.م
                </span>
              </div>
            ...
            <Button variant="hero" size="xl" className="w-full" onClick={() => setShowCart(true)}>
              <ShoppingCart className="w-5 h-5" /> السلة ({cartCount}) - {cartTotal} ج.م
            </Button>
          )}
        </motion.div>
      )}
